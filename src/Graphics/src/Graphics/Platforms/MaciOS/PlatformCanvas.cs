...
namespace Microsoft.Maui.Graphics.Platform
{
	public partial class PlatformCanvas : AbstractCanvas<PlatformCanvasState>
	{
		...
		private void FillWithImage(nfloat x, nfloat y, Action drawingAction)
		{
			_context.SaveState();
#if !MONOMAC
			// iOS fix: the pattern phase is pre-set to (Frame.X, Frame.Y) by PlatformGraphicsView,
			...
			// (macOS computes its phase differently and is already correct, so skip it there.)
